...
- File audit output persists at `/srv/polinetwork/state/openbao/audit`, mounted
  at `/openbao/logs`. The `persistent-file` audit device is declared in the
  server configuration because OpenBao 2.5 rejects API-created audit devices.
...
- `pn-secrets` is an external, internal-only Docker network shared across
  Compose projects. OpenBao and dedicated OpenBao Agent sidecars join it;
  application containers do not. OpenBao itself also joins `pn-edge` for the
  Access-protected UI/API route.
